        ...
        <div class="flex-column txt-content">
          <div class="flex-row">
            ...
            <div class="flex-column txt-content nowrap">
              <strong>{{articleData.typename}}</strong>
            </div>
          </div>
        </div>
        <div class="flex-column txt-content">
      ...
      <div class="comment-content">
        ...
        <div class="comment-list" v-if="commentList.length>0">
          <div class="item-comment" v-for="(item,index) in commentList" v-bind:key="item.id">
            <div class="comment-header"><span class="floor">#{{commentList.length-index}}楼</span><span class="time">{{item.addtime}}</span><span class="name" v-if="item.is_hidename==0">{{item.name}}：</span><span class="name" v-else>匿名：</span><span v-if="articleData.commentdelete==1&&menuButtonPermit.includes('Article_commentdelete')" class="delete" v-on:click="deleteComment(item.id)" title="删除该条评论"><i class="el-icon-delete-solid"></i></span></div>
