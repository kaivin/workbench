                        ...
                        <div slot="header">
                            <div class="card-header" ref="headerPane">
                                <div class="search-wrap" ref="searchPane">
                                    ...
                                    <div class="item-search">
                                      <el-input v-model="searchData.depart" placeholder="请输入部门名称" @change="searchResult" size="small" clearable></el-input>
                                      <!-- <el-select style="width: 140px;" v-model="searchData.depart" placeholder="请选择部门" size="small" clearable @change="searchResult">
                                        <el-option v-for="item in departList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                                      </el-select> -->
                                    </div>
                                    <div class="item-search">
                                        <el-input v-model="searchData.name" placeholder="请输入用户名" @change="searchResult" size="small" clearable></el-input>
                                    </div>
                                    <div class="item-search">
                                        ...
                                        <el-button class="item-input" plain type="info" size="small" @click="resetSearch">重置</el-button>
                                        <el-button type="primary" plain class="resetBtn" size="small" v-if="menuButtonPermit.includes('Chinacount_chexportlinkgroup')">
                                          <label>
                                        ...
                                        <el-link target="_blank" tag='a' href='https://mgl.hxjq.org/hxindex/Chinacount/chgettemplategroup' v-if="menuButtonPermit.includes('Chinacount_chgettemplategroup')" class="ImportTemplate">下载导入模板</el-link> 
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
  ...
  methods:{
    ...
    searchDataInit(){
      ...
      searchData.departname = $this.searchData.depart;
      searchData.uname = $this.searchData.name;
      return searchData;
    },
    // 初始化数据
    initData(){
    ...
    initPage(){
      ...
      $this.$store.dispatch('chinadeal/expenseData', searchData).then(res=>{
        if(res){
          if(res.status){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Chinacount_chmoneylist')){
              $this.initPage();
              // $this.getDepartList();
            }else{
              $this.$message({
    ...
    handleExportload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['ID','日期','部门', '姓名','账户名','渠道','类型', '消费']
        ...
        list.forEach(function(item,index){
          ...
          itemData.push(item.depart);
          itemData.push(item.name);
          itemData.push(item.pushname);
          itemData.push(item.moneytype);
    ...
    ...mapActions({
      setExpenseaddAction:'chinadeal/expenseAdd'
    }),
    //导入数据
    FileUp(e){
      var $this = this;
