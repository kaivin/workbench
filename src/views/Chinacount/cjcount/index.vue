                                ...
                                <el-table
                                    ...
                                    <el-table-column
                                      ...
                                      <template #default="scope">
                                        <span class="deal_score" v-if="scope.row.score > 0">{{scope.row.score.toFixed(1)}}</span>
                                        <span class="deal_score" v-else>{{scope.row.score}}</span>
                                        <!-- <span class="deal_num" v-if="scope.row.score > 0">({{getTotalNumber(scope.row)}}个)</span> -->
                                      </template>
                                    </el-table-column>
                                    <el-table-column
                                      label="询盘成交间隔期内成交分"
                                      ...
                                      <el-table-column
                                        ...
                                        >
                                        <template #default="scope">
                                          <span class="deal_score" v-if="scope.row.one_score > 0">{{scope.row.one_score.toFixed(1)}}</span>
                                          <span class="deal_score" v-else>{{scope.row.one_score}}</span>
                                          <!-- <span class="deal_num" v-if="scope.row.one_score > 0">({{scope.row.one_number}}个)</span> -->
                                        </template>
                                      </el-table-column>
                                      <el-table-column
                                        prop="two_score"
                                        ...
                                        >
                                        <template #default="scope">
                                          <span class="deal_score" v-if="scope.row.two_score > 0">{{scope.row.two_score.toFixed(1)}}</span>
                                          <span class="deal_score" v-else>{{scope.row.two_score}}</span>
                                          <!-- <span class="deal_num" v-if="scope.row.two_score > 0">({{scope.row.two_number}}个)</span> -->
                                        </template>
                                      </el-table-column>
                                      <el-table-column
                                        prop="three_score"
                                        ...
                                        >
                                        <template #default="scope">
                                          <span class="deal_score" v-if="scope.row.three_score > 0">{{scope.row.three_score.toFixed(1)}}</span>
                                          <span class="deal_score" v-else>{{scope.row.three_score}}</span>
                                          <!-- <span class="deal_num" v-if="scope.row.three_score > 0">({{scope.row.three_number}}个)</span> -->
                                        </template>
                                      </el-table-column>
                                      <el-table-column
                                        prop="four_score"
                                        ...
                                        >
                                        <template #default="scope">
                                          <span class="deal_score" v-if="scope.row.four_score > 0">{{scope.row.four_score.toFixed(1)}}</span>
                                          <span class="deal_score" v-else>{{scope.row.four_score}}</span>
                                          <!-- <span class="deal_num" v-if="scope.row.four_score > 0">({{scope.row.four_number}}个)</span> -->
                                        </template>
                                      </el-table-column>
                                      ...
                                      <el-table-column
                                        ...
                                        >
                                        <template #default="scope">
                                          <span class="deal_score" v-if="scope.row.five_score > 0">{{scope.row.five_score.toFixed(1)}}</span>
                                          <span class="deal_score" v-else>{{scope.row.five_score}}</span>
                                          <!-- <span class="deal_num" v-if="scope.row.five_score > 0">({{scope.row.five_number}}个)</span> -->
                                        </template>
                                      </el-table-column>
                                    </el-table-column>
...
export default {
  ...
  created(){
    ...
    $this.$store.dispatch('chinadeal/dealTimeData',null).then(res=>{
      if(res.status){
        ...
        $this.searchData.date = res.chtime;
  ...
  methods:{
    ...
    initPage(){
      ...
      $this.$store.dispatch('chinadeal/groupDealCycleData', searchData).then(res=>{
        if(res){
          if(res.status){
            $this.loading = false;
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Chinacount_cjcount')){
              $this.initPage();
            }else{
              $this.$message({
                showClose: true,
    ...
    getTotalNumber(data){
      return data.one_number+data.two_number+data.three_number+data.four_number+data.five_number;
    },
    getSummaries(param) {
