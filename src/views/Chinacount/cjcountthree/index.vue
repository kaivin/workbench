...
export default {
  ...
  created(){
    ...
    $this.$store.dispatch('chinadeal/dealTimeData',null).then(res=>{
      if(res.status){
        ...
        var month = new Date(res.chtime).getMonth()+1;
        var year = new Date(res.chtime).getFullYear();
        var startDate = "";
        if(month < 3){
        ...
        $this.searchData.date = [startDate,res.chtime];
  ...
  methods:{
    ...
    initPage(){
      ...
      $this.$store.dispatch('chinadeal/dealProductData', searchData).then(res=>{
        if(res){
          if(res.status){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Chinacount_cjcountthree')){
              $this.initPage();
            }else{
              $this.$message({
                showClose: true,
