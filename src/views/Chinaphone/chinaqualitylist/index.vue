<template>
  <div class="page-root" ref="boxPane">
                            ...
                            <el-table
                              ...
                              <el-table-column
                                ...
                                <template slot-scope="scope">
                                  <span v-if="scope.row.status == 1">多选</span>
                                   <span v-if="scope.row.status == 2">单选</span>
                                </template>
                              </el-table-column>
                              <el-table-column
                                prop="score"
                              ...
                              <el-table-column
                                ...
                                <template #default="scope">
                                  <div class="table-button">
                                    <el-button size="mini" @click="editTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Chinaphone_chinaqualityedit')">编辑</el-button>
                                    <el-button size="mini" @click="deleteTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Chinaphone_chinaqualitydelete')" type="info" plain>删除</el-button>
                                  </div>
                                </template>
                              </el-table-column>
                            </el-table>
      ...
      <el-dialog :title="dialogText" v-if="(menuButtonPermit.includes('Chinaphone_chinaqualityadd')||menuButtonPermit.includes('Chinaphone_chinaqualityedit'))" custom-class="add-edit-dialog" :before-close="handleClose" :visible.sync="dialogFormVisible" width="600px">
        <el-form :model="dialogForm">
          <div class="item-form">
            <el-form-item label="内容：" :label-width="formLabelWidth">
                <el-input type="textarea" :rows="2" v-model="dialogForm.content" ref="content" clearable></el-input>
            </el-form-item>
          </div>
          <div class="item-form">
          ...
          <div class="item-form">
            <el-form-item label="分数：" :label-width="formLabelWidth">
                <el-input v-model="dialogForm.score" ref="score" clearable></el-input>
            </el-form-item>
          </div>
          <div class="item-form">
...
export default {
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Chinaphone_chinaqualitylist')){
              var operationsWidth = 22;
              if($this.menuButtonPermit.includes('Chinaphone_chinaqualityedit')){
                operationsWidth+=66;
              }
              if($this.menuButtonPermit.includes('Chinaphone_chinaqualitydelete')){
                operationsWidth+=66;
              }
    ...
    handleClose(){
      ...
      $this.$store.dispatch('app/closeChinaQuality');
    },
    // 添加表格行数据
    ...
    editTableRow(row,index){
      var $this = this;
      $this.getSelectedList(row.id);
      $this.dialogFormVisible = true;
      $this.dialogText = "编辑产品质量判定条件";
      $this.resetFormData();
      $this.isNewAdd = false;
    ...
    saveData(){
      ...
      if(!$this.isSaveData){        
        ...
        var formData = {}
        formData.content = $this.dialogForm.content;
        ...
        if($this.isNewAdd){
          ...
          pathUrl = "chinaphone/getChinaQualityEdit";
        }
    ...
    deleteTableRow(row,index){
      ...
      $this.$confirm('是否确认删除该内容?', '提示', {
          ...
          $this.$store.dispatch('chinaphone/getChinaQualityDelete', {id:row.id}).then(response=>{
            if(response.status){
              $this.$message({
                showClose: true,
                message: response.info,
