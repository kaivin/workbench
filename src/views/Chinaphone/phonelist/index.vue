<template>
  <div class="page-root" ref="boxPane">
    <el-card class="box-card" shadow="hover">
      <div slot="header">
        <div class="card-header" ref="headerPane">
          ...
          <el-button type="primary" size="small" icon="el-icon-circle-plus-outline" @click="addTableRow()" v-if="device==='desktop'&&menuButtonPermit.includes('Chinaphone_phoneadd')">添加电话</el-button>
        </div>
      </div>
      <div class="card-content" ref="tableContent">
        <el-table
          ...
          <el-table-column
            v-if="(menuButtonPermit.includes('Chinaphone_getchinaphonereadrole')||menuButtonPermit.includes('Chinaphone_getchinaphonewriterole')||menuButtonPermit.includes('Chinaphone_getphonedomain')||menuButtonPermit.indexOf('Chinaphone_phoneedit')||menuButtonPermit.indexOf('Chinaphone_phonedelete'))&&device==='desktop'"
    ...
    <el-dialog :title="dialogText" v-if="(menuButtonPermit.includes('Chinaphone_phoneadd')||menuButtonPermit.includes('Chinaphone_phoneedit'))&&device==='desktop'" custom-class="add-edit-dialog" :visible.sync="dialogFormVisible" width="480px">
    ...
    <el-dialog :title="roleDialogText" v-if="(menuButtonPermit.includes('Chinaphone_getchinaphonereadrole')||menuButtonPermit.includes('Chinaphone_getchinaphonewriterole'))&&device==='desktop'" custom-class="user-dialog" :visible.sync="dialogRoleVisible" width="840px">
      <div class="user-role">
        <div class="role-wrap">
          <el-transfer 
            v-model="roleValue" 
    ...
    <el-dialog :title="domainDialogText" v-if="(menuButtonPermit.includes('Chinaphone_getphonedomain'))&&device==='desktop'" custom-class="user-dialog" :visible.sync="dialogDomainVisible" width="840px">
      <div class="user-role">
        <div class="role-wrap">
          <el-transfer 
            v-model="domainValue" 
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Chinaphone_phonelist')){
              var operationsWidth = 22;
              if($this.menuButtonPermit.includes('Chinaphone_getchinaphonereadrole')){
                operationsWidth+=114;
              }
              if($this.menuButtonPermit.includes('Chinaphone_getchinaphonewriterole')){
                operationsWidth+=114;
              }
              ...
              if($this.menuButtonPermit.includes('Chinaphone_phonedelete')){
                operationsWidth+=66;
              }
    ...
    deleteTableRow(row,index){
      ...
      $this.$confirm('是否确认'+tips+'该电话?', '提示', {
          ...
          $this.$store.dispatch('chinaphone/phoneDeleteAction', {id:row.id}).then(response=>{
            if(response.status){
              $this.$message({
                showClose: true,
                message: response.info,
    ...
    getSelectData(){
      ...
      $this.$store.dispatch('chinaphone/phoneAddEditDataAction', null).then(response=>{
          if(response.status){
            ...
            var phoneTypeList = [];
            response.phonetype.forEach(function(item,index){
              var itemData = {};
              itemData.value = item.id;
              itemData.label = item.name;
