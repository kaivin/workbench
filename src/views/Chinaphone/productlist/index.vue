      ...
      <el-form :model="dialogForm">
        ...
        <div class="item-form">
            <el-form-item label="产品类别：" :label-width="formLabelWidth">
                <el-select v-model="dialogForm.productlevel" clearable placeholder="请选择类别">
                    <el-option
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Chinaphone_productlist')){
              var operationsWidth = 22;
              if($this.menuButtonPermit.includes('Chinaphone_productedit')){
                operationsWidth+=66;
              }
              if($this.menuButtonPermit.includes('Chinaphone_productdelete')){
                operationsWidth+=66;
              }
