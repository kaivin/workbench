...
export default {
  ...
  methods: {
    ...
    getUserMenuButtonPermit(){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.data.length>0){
          ...
          if(permitData.includes('Commdepart_chinacount')){
              $this.getDepartList()
          }else{
            $this.$message({
              showClose: true,
    ...
    getDepartList(){
      var $this = this;
      $this.$store.dispatch('depcomm/depcomChDepartAction', null).then(res=>{
        var departList = [];
    ...
    getSearchData(){
      ...
      $this.$store.dispatch('depcomm/depcomChCountAction', searchData).then(res=>{
          if(res.status){
            var sourceData=[];
            //  询盘
            if(res.xunlist && res.xunlist.length > 0){
              ...
              var xunObj=$this.sourceRank(xunSource,'渠道询盘个数排行榜','个', res.xunlist, 0);
            ...
            if(res.scorelist && res.scorelist.length > 0){
              ...
              scoreSource.forEach(function(item){
                item.number = numSeparate(item.score);
              });
              var scoreObj=$this.sourceRank(scoreSource,'渠道询盘积分排行榜','分', res.scorelist, 1);
    ...
    sourceRank(dateArr,Name,Tag, xunlist, number){
      var itemObj={};
