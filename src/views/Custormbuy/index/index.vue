                      ...
                      <div class="card-header SaleCard" ref="headerPane">
                          <div class="search-wrap Compart-search-wrap" ref="searchPane">          
                              ...
                              <div class="item-search">
                                ...
                                <el-button type="primary" plain class="resetBtn" size="small" v-if="menuButtonPermit.includes('Custormbuy_exportlink')">
                                  <label>
                                    <span class="button-font">导入excel</span>
                                    <input style="display:none;" type="file" accept=".xlsx, .xls" @change="custormbuyFileUp"/>
                                  </label>
                                </el-button>
                                <el-link target="_blank" tag='a' href='https://mgl.hxjq.org/hxindex/Custormbuy/gettemplate' class="ImportTemplate">下载导入模板</el-link>                           
                              </div>
                          </div>
                      </div>
                    </div>
...
<script>
import { deparDealListChoose,deparDealEdit,deparDealListAdd,deparDealDel } from '@/api/Compare';
import { getCustormbuyList,setExportLink } from '@/api/Custormbuy';
...
export default {
  ...
  methods:{
    ...
    custormbuyFileUp(e){
      ...
      setExportLink(formData).then(res=>{
        $this.$message({
          showClose: true,
          message: res.info,
    ...
    dealData(){
      var $this = this;
      deparDealListChoose(null).then(response=>{
        if(response){
          if(response.status){
    ...
    initPage(){
      ...
      getCustormbuyList(formData).then(response=>{
        if(response){
          if(response.status){
            if(response.data.length>0){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Custormbuy_index')){
              $this.dealData();
            }else{
              $this.$message({
                showClose: true,
