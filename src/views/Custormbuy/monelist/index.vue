                            ...
                            <div class="card-header" ref="headerPane">
                                <div class="search-wrap" ref="searchPane">
                                    ...
                                    <div class="item-search">
                                        ...
                                        <el-button type="primary" plain class="resetBtn" size="small" v-if="menuButtonPermit.includes('Custormbuy_exportlinkgroup')">
                                          <label>
                                        ...
                                        <el-link target="_blank" tag='a' href='https://mgl.hxjq.org/hxindex/Custormbuy/gettemplategroup' v-if="menuButtonPermit.includes('Custormbuy_gettemplategroup')" class="ImportTemplate">下载导入模板</el-link> 
                                    </div>
                                </div>
                            </div>
                        </div>
...
import { getCustormbuyMoneylist,exportCustormbuyMoney } from '@/api/Custormbuy';
export default {
  name: 'Custormbuy_monelist',
  data() {
    return {
  ...
  methods:{
    ...
    initPage(){
      ...
      getCustormbuyMoneylist(searchData).then(res=>{
        if(res){
          if(res.status){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Custormbuy_monelist')){
    ...
    moneylistFileUp(e){
      ...
      exportCustormbuyMoney(formData).then(res=>{
        $this.$message({
          showClose: true,
          message: res.info,
