                            ...
                            <div class="card-header" ref="headerPane">
                                <div class="search-wrap" ref="searchPane">
                                    ...
                                    <div class="item-search">
                                        ...
                                        <el-button type="primary" plain class="resetBtn" size="small" v-if="menuButtonPermit.includes('Custormenbuy_exportlinkgroup')">
                                          <label>
                                            <span class="button-font">导入Excel</span>
                                            <input style="display:none;" type="file" accept=".xlsx, .xls" @change="moneylistFileUp"/>
                                          </label>
                                        </el-button>
                                        ...
                                        <el-link target="_blank" tag='a' href='https://mgl.hxjq.org/hxindex/Custormenbuy/gettemplategroup' v-if="menuButtonPermit.includes('Custormenbuy_gettemplategroup')" class="ImportTemplate">下载导入模板</el-link> 
                                    </div>
                                </div>
                            </div>
                        </div>
...
import { getEnCustormbuyMoneylist,exportEnCustormbuyMoney } from '@/api/Custormbuy';
export default {
  name: 'Custormenbuy_monelist',
  data() {
    return {
  ...
  methods:{
    ...
    initPage(){
      ...
      getEnCustormbuyMoneylist(searchData).then(res=>{
        if(res){
          if(res.status){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Custormenbuy_monelist')){
    ...
    moneylistFileUp(e){
      ...
      exportEnCustormbuyMoney(formData).then(res=>{
        $this.$message({
          showClose: true,
          message: res.info,
