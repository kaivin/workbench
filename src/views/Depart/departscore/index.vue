<template>
  <div class="page-root SaleCard" ref="boxPane">
                      ...
                      <div class="table-wrapper" v-bind:class="scrollPosition.isFixed?'fixed-table':''">
                        <div class="table-mask"></div>
                          <el-table
                              ...
                              <el-table-column
                                v-if="(menuButtonPermit.indexOf('Depart_departscoreedit')||menuButtonPermit.indexOf('Depart_departscoredelete'))"
                                ...
                                <template #default="scope">
                                  <div class="table-button">
                                    <el-button size="mini" @click="editTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Depart_departscoreedit')">编辑</el-button>
                                    <el-button size="mini" @click="deleteTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Depart_departscoredelete')" type="info" plain>删除</el-button>
                                  </div>
                                </template>
                              </el-table-column>
                          </el-table>
    ...
    <el-dialog :title="dialogText" v-if="(menuButtonPermit.includes('Depart_departscoreadd')||menuButtonPermit.includes('Depart_departscoreedit'))" custom-class="add-edit-dialog" :before-close="handleClose" :visible.sync="dialogFormVisible" width="440px">
      <el-form :model="dialogForm">
        ...
        <div class="item-form">
            <el-form-item label="部门：" :label-width="formLabelWidth">
                <el-select v-model="dialogForm.depart_id" clearable placeholder="请选择部门">
                    <el-option
        ...
        <div class="item-form">
            <el-form-item label="本月积分：" :label-width="formLabelWidth">
...
export default {
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Depart_departscore')){
              var operationsWidth = 22;
              if($this.menuButtonPermit.includes('Depart_departscoreedit')){
                operationsWidth+=66;
              }
              if($this.menuButtonPermit.includes('Depart_departscoredelete')){
                operationsWidth+=66;
              }
    ...
    handleClose(){
      ...
      $this.$store.dispatch('app/closeDepartScore');
    },
    // 添加表格行数据
    addTableRow(row,index){
      var $this = this;
      $this.dialogFormVisible = true;
      $this.dialogText = "添加部门月度积分";
      $this.resetFormData();
    },
    // 编辑表格行数据
    editTableRow(row,index){
      var $this = this;
      $this.dialogFormVisible = true;
      $this.dialogText = "编辑部门月度积分";
      $this.resetFormData();
      $this.dialogForm.id = row.id;
      $this.dialogForm.depart_id = row.depart_id;
