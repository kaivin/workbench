        ...
        <div class="item-filter flex-box group">
          ...
          <div class="filter-content flex-content">
            <div class="item-list group">
              ...
              <div class="item-checkbox" v-bind:class="item.isOn?'active':''" v-for="item in groupList" v-bind:key="item.id" v-on:click="groupChangeHandler(item.uname)">
            ...
            if(response.data){
              if(response.data.length > 0){
                ...
                response.data.forEach(item => {
                  if(!$this.findData(item.uname, resList, "uname")){
                    var obj = {};
                    obj.id = item.id;
                    obj.name = item.name;
