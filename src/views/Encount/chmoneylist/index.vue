                            ...
                            <div class="card-header" ref="headerPane">
                                <div class="search-wrap" ref="searchPane">
                                    ...
                                    <div class="item-search">
                                        ...
                                        <el-button type="primary" plain class="resetBtn" size="small" v-if="menuButtonPermit.includes('Encount_chexportlinkgroup')">
                                          <label>
                                        ...
                                        <el-link target="_blank" tag='a' href='https://mgl.hxjq.org/hxindex/Encount/chgettemplategroup' v-if="menuButtonPermit.includes('Encount_chgettemplategroup')" class="ImportTemplate">下载导入模板</el-link> 
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
  ...
  methods:{
    ...
    initPage(){
      ...
      $this.$store.dispatch('endeal/expenseData', searchData).then(res=>{
        if(res){
          if(res.status){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Encount_chmoneylist')){
              $this.initPage();
              // $this.getDepartList();
            }else{
              $this.$message({
    ...
    ...mapActions({
      setExpenseaddAction:'endeal/expenseAdd'
    }),
    //导入数据
    FileUp(e){
      var $this = this;
