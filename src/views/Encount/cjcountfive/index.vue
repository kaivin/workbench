...
export default {
  ...
  methods:{
    ...
    initPage(){
      ...
      $this.$store.dispatch('endeal/futureDealData', null).then(res=>{
        if(res){
          if(res.status){
              if(res.data && res.data.length > 0){
                res.data.forEach(item => {
                  item.cj_number_one = item.xunlist[0].yc_allnumber;
                  item.cj_number_two = item.xunlist[1].yc_allnumber;
                  item.cj_number_three = item.xunlist[2].yc_allnumber; 
                  item.cj_number_four = item.xunlist[3].yc_allnumber;
                  item.cj_number_five = item.xunlist[4].yc_allnumber;
                  item.cj_number_six = item.xunlist[5].yc_allnumber;
                })
              }
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            $this.getMonthList();
            if($this.menuButtonPermit.includes('Encount_cjcountfive')){
              $this.initPage();
            }else{
              $this.$message({
                showClose: true,
    ...
    getMonthList(){
      ...
      var month = [];
      for(var i = 0; i < 6; i++){
        month.push($this.getMonthAfter(i));
      }
    ...
    getMonthAfter(months) {
      ...
      // 返回结果
      return nextYear+'-'+('0'+nextMonth).slice(-2);
    },
    // 设置横向滚动条相关DOM数据
    setScrollDom(){
      var $this = this;
