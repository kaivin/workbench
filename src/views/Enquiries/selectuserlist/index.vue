  ...
  <div class="page-root SaleCard" ref="boxPane">
                      ...
                      <div class="card-header SaleCard" ref="headerPane">
                          <div class="search-wrap Compart-search-wrap" ref="searchPane">          
                              <div class="item-search" style="width:200px;">                              
                                    <el-date-picker 
                                    v-model="searchData.mtime" format="yyyy-MM" value-format="yyyy-MM" 
                                    type="month"
                                    size="small"
                                    placeholder="选择月">
                                    </el-date-picker>
                              ...
                              <div class="item-search">
                                  <el-select size="small" filterable v-model="searchData.dept_id" clearable placeholder="请选择部门">
                                    <el-option
                              ...
                              <el-table-column
                                ...
                                <template #default="scope">
                                  <div class="table-button">
                                    <el-button size="mini" @click="delTableRow(scope.row)" v-if="menuButtonPermit.includes('Enquiries_selectuserdel')">删除</el-button>
                                  </div>
                                </template>
                              </el-table-column>
                          </el-table>
      ...
      <el-dialog :title="dialogText" v-if="menuButtonPermit.includes('Enquiries_selectuseradd')" custom-class="add-edit-dialog" :before-close="handleClose" :visible.sync="dialogFormVisible" width="680px">
        <el-form :model="dialogForm">
          <div class="item-form-group">
                <div class="item-form Compart-time-width">
                    <el-form-item label="添加时间：" :label-width="formLabelWidth">
                        <el-date-picker v-model="dialogForm.mtime" style="widht:100%" format="yyyy-MM" value-format="yyyy-MM" type="month" size="small" placeholder="选择月"></el-date-picker>
                    </el-form-item>
                </div>
                <div class="item-form Compart-time-width">
                    <el-form-item label="员工组：" :label-width="formLabelWidth">
                            <el-select filterable v-model="dialogForm.is_old" clearable placeholder="请选择员工组">
                                <el-option
          ...
          <div class="item-form-group">
              <div class="item-form Compart-time-width">
                  <el-form-item label="部门" :label-width="formLabelWidth">
                      <el-select filterable v-model="dialogForm.dept_id" clearable placeholder="请选择部门" @change="handerDept">
                          <el-option
                              v-for="item in deptList"
                              :key="item.value"
                              :label="item.label"
         ...
         <div class="item-form Compart-time-width" v-if="departUser.length>0">
            <el-form-item label="组员" :label-width="formLabelWidth">                  
            <el-checkbox-group v-model="dialogForm.uid">
                <el-checkbox 
                v-for="item in departUser"
                :key="item.value"
                :label="item.value">
                {{item.label}}
                </el-checkbox>
...
export default {
  ...
  created(){
    var $this = this;
    $this.searchData.mtime=$this.getMonth();
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Enquiries_selectuserlist')){
              $this.getDeptList();
            }else{
              $this.$message({
                showClose: true,
    ...
    restSearch(){
      ...
      if($this.searchData.mtime&&$this.searchData.mtime!=''){
        formData.mtime = $this.searchData.mtime;
      }
      return formData;
    },
    // 初始化页面信息
    ...
    handleClose(){
      ...
      $this.$store.dispatch('app/closeSelectuser');
    },
    // 添加表格行数据
    addTableRow(row,index){
      var $this = this;
      $this.dialogFormVisible = true;
      $this.dialogText = "添加成员";
      $this.resetFormData();
    },
    // 保存添加/编辑数据
    saveData(){
