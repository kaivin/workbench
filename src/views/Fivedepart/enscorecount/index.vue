<template>
  ...
  import sourceRank from "../components/ensourcecount/sourceRank.vue";
  import {randomString,numSeparate,sortByDesc,rankingWithTotalItem,singleArrRepeatColor} from "@/utils/index";
  export default {
    name: 'Fivedepart_enscorecount',
    components: {
      sourceRank,
    },
    data() {
    ...
    methods: {
      ...
      getUserMenuButtonPermit(){
        ...
        $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
          if(res.data.length>0){
            ...
            if(permitData.includes('Fivedepart_enscorecount')){
                $this.getDepartList()
            }else{
              $this.$message({
                showClose: true,
      ...
      getDepartList(){
        var $this = this;
        $this.$store.dispatch('depfive/depfiveEnConditionAction', null).then(res=>{
          var departList = [];
      ...
      departDataRes(dataArr, Tag){
        ...
        var itemArr=singleArrRepeatColor(dataArr);
      ...
      getSearchData(){
        ...
        $this.$store.dispatch('depfive/depfiveEnScoreDataAction', searchData).then(res=>{
            if(res.status){
              var sourceData=[];
              if(res.proscorelist && res.proscorelist.length > 0){
                var proscorelist=$this.departDataRes(res.proscorelist,'score');
                var scoreSource=rankingWithTotalItem(proscorelist,'value');
                var allnum = 0;
                scoreSource.forEach(function(item){
                  allnum += Number(item.value);
                  item.name = item.xunproduct;
                  item.number = numSeparate(item.value);
                });
                var scoreObj=$this.sourceRank(scoreSource,'产品积分排行榜','分', allnum);
                sourceData.push(scoreObj);
              }
              if(res.contient_scorelist && res.contient_scorelist.length > 0){
                var contient_scorelist=$this.departDataRes(res.contient_scorelist,'score');
                var scoreSource=rankingWithTotalItem(contient_scorelist,'value');
                var allnum = 0;
                scoreSource.forEach(function(item){
                  allnum += Number(item.value);
                  item.name = item.contient;
                  item.number = numSeparate(item.value);
                });
                var scoreObj=$this.sourceRank(scoreSource,'大洲积分排行榜','分', allnum);
                sourceData.push(scoreObj);
              }
