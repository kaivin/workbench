  ...
  export default {
    ...
    methods: {
      ...
      getUserMenuButtonPermit(){
        ...
        $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
          if(res.data.length>0){
            ...
            if(permitData.includes('Fivedepart_ensourcecount')){
                $this.getDepartList()
            }else{
              $this.$message({
                showClose: true,
      ...
      getSearchData(){
        ...
        $this.$store.dispatch('depfive/depfiveEnSourceDataAction', searchData).then(res=>{
            if(res.status){
              var sourceData=[];
              var allnum = 0;
              if(res.list && res.list.length > 0){
                var xunlist=$this.departDataRes(res.list,'xunnumber');
                var scoreSource=rankingWithTotalItem(xunlist,'value');
                scoreSource.forEach(function(item){
                  allnum += Number(item.value);
                  item.name = item.remark2;
                  item.number = numSeparate(item.value);
                });
                var scoreObj=$this.sourceRank(scoreSource,'渠道询盘排行榜','个', allnum);
