        ...
        console.log(response,"默认数据");
          if(response){
            if(response.status){
                ...
                dealCountData.chartTitle = "部门成交个数月趋势";
                ...
                dealCountData.nowNumber = response.scorenumberall;
                ...
                dealCountData.nowLastNumber = Math.abs(response.scorenumberall - response.lastscorenumberall);
                ...
                dealCountData.avgNumber = response.avgscorenumber;
                ...
                dealCountData.historyMaxNumber = response.historyscorenumber[0].number;
      ...
      if(!obj.isOn){
        ...
        contrastList.forEach(function(item,index){
          if($this.selectedData.isMonth){
            ...
            if(item.value=="dealScore"||item.value=="costCount"||item.value=="dealScoreRegion"||item.value=="dealCount"||item.value=="dealCountRegion"||item.value=="product"){
              item.isOn = false;
              item.disabled = true;
            }else{
              item.disabled = false;
      ...
      if($this.selectedType.includes("dealCount")){
        ...
        if($this.selectedData.isDateCompare&&$this.selectedData.dateContrast&&$this.selectedData.dateContrast.length>0){
          ...
          if($this.selectedData.groupID.length==1){
            $this.groupList.forEach(function(item){
              if(item.id == $this.selectedData.groupID[0]){
                dealCountData.chartTitle = item.name+"成交个数日期对比趋势";
                dealCountData.name = item.name+"成交个数统计";
              }
            });
          }else{
            dealCountData.chartTitle = "部门组合总成交个数日期对比趋势";
            dealCountData.name = "部门组合成交个数统计";
          }
        }else{
          // 小组对比
          if($this.selectedData.contrastGroupID.length>0){
            dealCountData.chartTitle = "各部门成交个数趋势对比";
            dealCountData.mainData = [];
            dealCountData.colorArr = [];
            ...
            if($this.selectedData.groupID.length==1){
              $this.groupList.forEach(function(item){
                if(item.id == $this.selectedData.groupID[0]){
                  dealCountData.chartTitle = item.name+"成交个数趋势";
                  dealCountData.name = item.name+"成交个数统计";
                }
              });
            }else{// 多小组被选中
              dealCountData.chartTitle = "部门组合成交个数趋势";
              dealCountData.name = "部门组合成交个数统计";
              dealCountData.totalChart = [];
