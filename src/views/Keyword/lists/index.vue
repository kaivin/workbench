<template>
  <div class="page-root" ref="boxPane">
              ...
              <div class="card-header" ref="headerPane">
                <div class="search-wrap" ref="searchPane">
                  <div class="item-search">
                    <el-select v-model="searchData.producttype_id" size="small" filterable clearable placeholder="请选择产品分类" class="select-panel">
                        <el-option
                  ...
                  <div class="item-search">
                    ...
                    <el-button v-if="menuButtonPermit.includes('Keyword_add')" type="success" size="small" icon="el-icon-plus" v-on:click="addTableRow()">新增搜索词</el-button>
                  </div>
                </div>
              </div>
            </div>
                      ...
                      <el-table-column
                        ...
                        <template #default="scope">
                          <div class="table-button">
                            <el-button size="mini" @click="editTableRow(scope.row, scope.$index)" v-if="menuButtonPermit.includes('Keyword_edit')" >编辑</el-button>
                            <el-button size="mini" @click="deleteTableRow(scope.row, scope.$index)" v-if="menuButtonPermit.includes('Keyword_delete')" type="info" plain >删除</el-button>
                          </div>
                        </template>
                      </el-table-column>
                    </el-table>
    ...
    <el-dialog :title="dialogText" v-if="(menuButtonPermit.includes('Keyword_add')||menuButtonPermit.includes('Keyword_edit'))" custom-class="add-edit-dialog" :visible.sync="visible" :before-close="handleClose" width="500px">
      <el-form :model="dialogForm">
        <el-form-item label="搜索词名称：" :label-width="formLabelWidth">
          <el-input v-model="dialogForm.name" ref="name" placeholder="请输入搜索词名称"></el-input>
        </el-form-item>
        <el-form-item label="分类：" :label-width="formLabelWidth">
          <el-select v-model="dialogForm.producttype_id" clearable placeholder="请选择搜索词分类">
            <el-option
                v-for="item in productTypeList"
                :key="item.id"
                :label="item.name"
...
export default {
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Keyword_lists')){
                var operationsWidth = 22;
                if($this.menuButtonPermit.includes('Keyword_edit')){
                  operationsWidth+=66;
                }
                if($this.menuButtonPermit.includes('Keyword_delete')){
                  operationsWidth+=66;
                }
    ...
    // 获取产品分类数据
    getProductTypeList() {
      base.productType(null).then(res => {
        console.log(res, '产品分类数据')
    ...
    initPage(){
      ...
      formData.producttype_id = $this.searchData.producttype_id;
      document.getElementsByClassName("scroll-panel")[0].scrollTop = 0;
      indices.keyword.search(formData).then(response=>{
        console.log(response, '搜索词列表')
          if(response){
            if(response.status){
              if(response.data){
    ...
    saveData() {
      ...
      if(!$this.isSaveData){
        ...
        if ($this.dialogForm.id === 0) {
          request = indices.keyword.add
        } else {
          request = indices.keyword.edit
        }
    ...
    validationForm() {
      ...
      if ($this.dialogForm.name == "") {
        $this.$message({
          showClose: true,
          message: "错误：搜索词名称不能为空！",
          type: "error",
        });
        $this.$refs["name"].focus();
        return false;
      }
      if ($this.dialogForm.producttype_id == "") {
        $this.$message({
          showClose: true,
          message: "错误：搜索词分类不能为空！",
          type: "error",
        });
        return false;
      }
    ...
    deleteTableRow(row, index) {
      ...
      $this.$confirm('是否确认删除该数据?', '提示', {
        ...
        indices.keyword.delete({ id: row.id }).then((response) => {
          if (response.status) {
            $this.$message({
              showClose: true,
              message: response.info,
