<template>
  <div class="page-root" ref="boxPane">
      ...
      <div class="scroll-panel" ref="scrollDom" style="will-change:scroll-position">
        <div class="true-height" ref="scrollPane">
          ...
          <el-card class="box-card" shadow="hover">
              ...
              <div class="card-header" ref="headerPane">
                <div class="search-wrap" ref="searchPane">
                  ...
                  <div class="item-search">
                    ...
                    <el-button v-if="menuButtonPermit.includes('Kuaishou_dyadd')" type="success" size="small" icon="el-icon-plus" v-on:click="addTableRow()">新增账号</el-button>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="table-wrapper" v-bind:class="scrollPosition.isFixed?'fixed-table':''">
                <div class="table-mask"></div>
                    <el-table
                      ...
                      <el-table-column
                        v-if="menuButtonPermit.includes('Kuaishou_dyedit')"
                        ...
                        <template #default="scope">
                          <div class="table-button">
                            <el-button size="mini" v-if="menuButtonPermit.includes('Kuaishou_dyedit')" @click="editTableRow(scope.row)" plain >编辑</el-button>
                          </div>
                        </template>
                      </el-table-column>
                    </el-table>
            ...
            <div class="pagination-panel" v-if="totalDataNum>50 && false" ref="pagePane">
              <el-pagination
    ...
    <el-dialog :title="dialogText" v-if="menuButtonPermit.includes('Kuaishou_dyadd') || menuButtonPermit.includes('Kuaishou_dyedit')" custom-class="add-edit-dialog" :visible.sync="visible" :before-close="handleClose" width="500px">
      <el-form :model="dialogForm">
        <el-form-item label="快手名称：" :label-width="formLabelWidth">
          <el-input v-model="dialogForm.name" ref="name" placeholder="请输入快手名称" clearable></el-input>
        </el-form-item>
        <el-form-item label="部门名称：" :label-width="formLabelWidth">
          <el-input v-model="dialogForm.department" ref="department" placeholder="请输入部门名称"></el-input>
        </el-form-item>
        <el-form-item label="快手负责人：" :label-width="formLabelWidth">
          <el-input v-model="dialogForm.uname" ref="uname" placeholder="请输入负责人姓名"></el-input>
        </el-form-item>
        <el-form-item label="快手uid：" :label-width="formLabelWidth">
          <el-input v-model="dialogForm.uuid" ref="uuid" placeholder="请输入快手uid"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
...
export default {
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Kuaishou_dyaccount')){
                var operationsWidth = 22;
                if($this.menuButtonPermit.includes('Kuaishou_dyedit')){
                  operationsWidth+=66;
                }
    ...
    initPage(){
      ...
      formData.department = $this.searchData.department;
      document.getElementsByClassName("scroll-panel")[0].scrollTop = 0;
      $this.$store.dispatch('kuaishou/kuaishouAccountList',formData).then(response=>{
        if(response){
          if(response.status){
            if(response.data){
    ...
    addTableRow() {
      this.visible = true;
      this.dialogText = "新增账号";
      this.resetFormData();
    },
    // 编辑表格行数据
    editTableRow(row) {
      this.visible = true;
      this.dialogText = "编辑账号";
      this.dialogForm.name = row.name;
      this.dialogForm.department = row.department;
      this.dialogForm.uname = row.uname;
      this.dialogForm.uuid = row.uuid;
    ...
    saveData() {
      ...
      if(!$this.isSaveData){
        ...
        result.department = $this.dialogForm.department;
        result.uname = $this.dialogForm.uname;
        result.uuid = $this.dialogForm.uuid;
        let link = "";
        if($this.dialogForm.id){
          result.id = $this.dialogForm.id;
          link="kuaishou/editKuaishouAccount"
        }else{
          link="kuaishou/addKuaishouAccount"
        }
        $this.$store.dispatch(link,result).then(response=>{
          if (response.status) {
            $this.$message({
              showClose: true,
              message: response.info,
    ...
    resetFormData() {
      ...
      $this.dialogForm.uname = "";
      $this.dialogForm.uuid = "";
      $this.dialogForm.id = "";
    },
    // 验证是否为空
    validationForm() {
      ...
      if ($this.dialogForm.uname == "") {
        $this.$message({
          showClose: true,
          message: "错误：快手负责人不能为空！",
          type: "error",
        });
        $this.$refs["uname"].focus();
        return false;
      }
      if ($this.dialogForm.uuid == "") {
        $this.$message({
          showClose: true,
          message: "错误：快手uid不能为空！",
          type: "error",
        });
        $this.$refs["uuid"].focus();
        return false;
      }
      return true;
    },
    ...
    deleteTableRow(row, index) {
      ...
      $this.$confirm('是否确认删除该数据?', '提示', {
        ...
        $this.$store.dispatch('kuaishou/kuaishouKeywordDel',{ id: row.id }).then(response=>{
          if (response.status) {
            $this.$message({
              showClose: true,
              message: response.info,
