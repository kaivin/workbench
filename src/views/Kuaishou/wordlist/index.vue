<template>
  <div class="page-root" ref="boxPane">
              ...
              <div class="card-header" ref="headerPane">
                <div class="search-wrap" ref="searchPane">
                  ...
                  <div class="item-search">
                    ...
                    <el-button v-if="menuButtonPermit.includes('Kuaishou_wordadd')" type="success" size="small" icon="el-icon-plus" v-on:click="addTableRow()">新增关键词</el-button>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="table-wrapper" v-bind:class="scrollPosition.isFixed?'fixed-table':''">
                <div class="table-mask"></div>
                    <el-table
                      ...
                      <el-table-column
                        v-if="menuButtonPermit.includes('Kuaishou_worddel')"
    ...
    <el-dialog :title="dialogText" v-if="menuButtonPermit.includes('Kuaishou_wordadd')" custom-class="add-edit-dialog" :visible.sync="visible" :before-close="handleClose" width="500px">
...
export default {
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Kuaishou_wordlist')){
                var operationsWidth = 22;
                if($this.menuButtonPermit.includes('Kuaishou_wordedit')){
                  operationsWidth+=66;
                }
                if($this.menuButtonPermit.includes('Kuaishou_worddel')){
                  operationsWidth+=66;
                }
    ...
    initPage(){
      ...
      $this.$store.dispatch('kuaishou/getKuaishouKeywordList',formData).then(response=>{
        if(response){
          if(response.status){
            if(response.data){
    ...
    saveData() {
      ...
      if(!$this.isSaveData){
        ...
        $this.$store.dispatch('kuaishou/kuaishouKeywordAdd',result).then(response=>{
          if (response.status) {
            $this.$message({
              showClose: true,
              message: response.info,
