<template>
  <div class="page-root" ref="boxPane">
                    ...
                    <div class="card-content" ref="tableContent">
                      <div class="table-wrapper">
                          ...
                          <el-table
                            ...
                            <el-table-column
                              v-if="(menuButtonPermit.indexOf('Ownpush_chscoretargetedit'))"
                              align="center"
                              fixed="right"
                              prop="operations"
                              label="操作">
                              <template #default="scope">
                                <div class="table-button">
                                  <el-button size="mini" @click="editTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Ownpush_chscoretargetedit')">编辑</el-button>
                                  <el-button size="mini" @click="deleteTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Ownpush_chscoretargetdelete')">删除</el-button>
                                </div>
                              </template>
                            </el-table-column>
                          </el-table>
      ...
      <el-dialog :title="dialogText" v-if="(menuButtonPermit.includes('Ownpush_chscoretargetadd')||menuButtonPermit.includes('Ownpush_chscoretargetedit'))" custom-class="add-edit-dialog" :visible.sync="dialogFormVisible" :before-close="handleClose" width="386px" center>
        <el-form :model="dialogForm">
            ...
            <el-form-item label="截止时间：" :label-width="formLabelWidth">
                <el-date-picker type="month" format="yyyy-MM" value-format="yyyy-MM" placeholder="选择截止时间" v-model="dialogForm.endtime" ref="endtime"></el-date-picker>
            </el-form-item>
...
export default {
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Ownpush_chscoretargetlist')){
               $this.initPage();
            }else{
              $this.$message({
                showClose: true,
    ...
    handleClose(){
      ...
      $this.$store.dispatch('app/closechAimScore');
    },
    // 添加表格行数据
