<template>
  <div class="page-root user-index" ref="boxPane">
                  ...
                  <div slot="header">
                    <div class="card-header" ref="headerPane">
                      <div class="search-wrap" ref="searchPane">
                        ...
                        <div class="item-search">
                          <el-select v-model="searchData.resourcetype_id" size="small" clearable placeholder="请选择资源分类" class="select-panel">
                            <el-option
                        ...
                        <div class="item-search">
                          <el-select v-model="searchData.dept_id" size="small" clearable placeholder="请选择部门" class="select-panel">
                            <el-option
                        ...
                        <div class="item-search">
                          <el-input
                            class="input-panel"
                            size="small"
                            placeholder="账号|手机|邮箱 模糊搜索"
                            v-model="searchData.keyworkd"
                          ...
                          <el-table
                            ...
                            <el-table-column
                              ...
                              <template slot-scope="scope">
                                <div class="table-tag">
                                    <el-tag v-if="scope.row.resourcetype_id==2">虚拟资源</el-tag>
                                    <el-tag type="success" v-else>实物资源</el-tag>
                                </div>
                                </template>
                            </el-table-column>
                            <el-table-column
                            ...
                            <el-table-column
                              v-if="(menuButtonPermit.indexOf('Resource_resourceedit')||menuButtonPermit.indexOf('Resource_resourcedelete'))"
                              ...
                              <template #default="scope">
                                <div class="table-button">
                                  <el-button size="mini" @click="editTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Resource_resourceedit')">编辑</el-button>
                                  <el-button size="mini" @click="deleteTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Resource_resourcedelete')" type="info" plain>删除</el-button>
                                </div>
                              </template>
                            </el-table-column>
                          </el-table>
      ...
      <el-dialog :title="dialogText" v-if="(menuButtonPermit.includes('Resource_resourceadd')||menuButtonPermit.includes('Resource_resourceedit'))" custom-class="add-edit-dialog" :visible.sync="dialogFormVisible" :before-close="handleClose" width="680px">
        <el-form :model="dialogForm">
          <div class="item-form-group">
            <div class="item-form">
              <el-form-item label="账号：" :label-width="formLabelWidth">
                <el-input v-model="dialogForm.account" ref="account"></el-input>
              </el-form-item>
            </div>
            <div class="item-form">
              <el-form-item label="密码：" :label-width="formLabelWidth">
                <el-input v-model="dialogForm.pwd" ref="pwd"></el-input>
              </el-form-item>
            </div>
          </div>
          <div class="item-form-group">
          ...
          <div class="item-form-group">
            <div class="item-form">
              <el-form-item label="资源分类：" :label-width="formLabelWidth">
                <el-select v-model="dialogForm.resourcetype_id" clearable @change="cateChangeHandler" placeholder="请选择资源分类">
                  <el-option
                    v-for="item in cateList"
                    :key="item.value"
                    :label="item.label"
            ...
            <div class="item-form">
              <el-form-item label="资源类型：" :label-width="formLabelWidth">
                <el-select v-model="dialogForm.resourcetypeson_id" clearable placeholder="请选择资源类型">
                  <el-option
          ...
          <div class="item-form">
            <el-form-item label="负责人：" :label-width="formLabelWidth">
                <el-select v-model="dialogForm.uid" clearable filterable placeholder="请选择负责人">
                    <el-option
          ...
          <div class="item-form">
              <el-form-item label="备注：" :label-width="formLabelWidth">
                <el-input type="textarea" v-model="dialogForm.remark" :autosize="{ minRows: 2, maxRows: 4}" ref="remark"></el-input>
              </el-form-item>
          </div>
        </el-form>
        <template #footer>
...
export default {
  ...
  methods:{
    ...
    initPage(){
      ...
      formData.keyworkd = $this.searchData.keyworkd;
      formData.limit = $this.searchData.limit;
      formData.resourcetype_id = $this.searchData.resourcetype_id;
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Resource_index')){
              var operationsWidth = 22;
              if($this.menuButtonPermit.includes('Resource_resourceedit')){
                operationsWidth+=66;
              }
              if($this.menuButtonPermit.includes('Resource_resourcedelete')){
                operationsWidth+=66;
              }
    ...
    handleClose(){
      ...
      $this.$store.dispatch('app/closeResource');
    },
    // 添加表格行数据
    addTableRow(row,index){
      var $this = this;
      $this.dialogFormVisible = true;
      $this.dialogText = "添加资源";
      $this.resetFormData();
    },
    // 编辑表格行数据
    editTableRow(row,index){
      var $this = this;
      $this.dialogFormVisible = true;
      $this.dialogText = "修改资源";
      $this.resetFormData();
      $this.cateChangeHandler(row.resourcetype_id);
      ...
      $this.dialogForm.pwd = row.pwd;
      $this.dialogForm.resourcetypeson_id = row.resourcetypeson_id;
      $this.dialogForm.resourcetype_id = row.resourcetype_id;
    ...
    saveData(){
      ...
      if(!$this.isSaveData){
        ...
        formData.account = $this.dialogForm.account;
        formData.pwd = $this.dialogForm.pwd;
        formData.remark = $this.dialogForm.remark;
        formData.resourcetypeson_id = $this.dialogForm.resourcetypeson_id;
        formData.resourcetype_id = $this.dialogForm.resourcetype_id;
    ...
    resetSearchData(){
      var $this = this;
      $this.searchData.keyworkd = "";
      $this.searchData.page = 1;
      $this.searchData.limit = 20;
      $this.searchData.dept_id = "";
      $this.searchData.resourcetype_id = "";
    ...
    resetFormData(){
      ...
      $this.dialogForm.resourcetypeson_id = "";
      $this.dialogForm.resourcetype_id = "";
      $this.dialogForm.uid = "";
    },
    // 验证是否为空
    validationForm(){
      var $this = this;
      if($this.dialogForm.account == ""&&$this.dialogForm.pwd == ""&&$this.dialogForm.phone == ""&&$this.dialogForm.email == ""){
        $this.$message({
            showClose: true,
            message: '错误：账号、密码、手机号、邮箱不能同时为空！',
            type: 'error'
        });
        return false;
      }
      if($this.dialogForm.resourcetype_id == ""){
        $this.$message({
            showClose: true,
            message: '错误：请选择资源所属分类！',
            type: 'error'
        });
        return false;
      }
      if($this.dialogForm.resourcetypeson_id == ""){
        $this.$message({
            showClose: true,
            message: '错误：请选择资源所属类型！',
            type: 'error'
        });
        return false;
      }
