<template>
  <div class="page-root" ref="boxPane">
                    ...
                    <div class="card-content" ref="tableContent">
                      <div class="table-wrapper" v-bind:class="scrollPosition.isFixed?'fixed-table':''">
                          ...
                          <el-table
                            ...
                            <el-table-column
                              v-if="(menuButtonPermit.includes('Resource_typeedit')||menuButtonPermit.includes('Resource_typedelete'))"
                              ...
                              <template #default="scope">
                                <div class="table-button">
                                  <el-button size="mini" @click="editTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Resource_typeedit')">编辑</el-button>
                                  <el-button size="mini" @click="deleteTableRow(scope.row,scope.$index)" v-if="menuButtonPermit.includes('Resource_typedelete')" type="info" plain>删除</el-button>
                                </div>
                              </template>
                            </el-table-column>
                          </el-table>
    ...
    <el-dialog :title="dialogText" v-if="(menuButtonPermit.includes('Resource_typeadd')||menuButtonPermit.includes('Resource_typeedit'))" custom-class="add-edit-dialog" :visible.sync="dialogFormVisible" :before-close="handleClose" width="480px">
      <el-form :model="dialogForm">
        <div class="item-form">
          <el-form-item label="资源类型名：" :label-width="formLabelWidth">
            <el-input v-model="dialogForm.name" ref="name"></el-input>
          </el-form-item>
        </div>
        <div class="item-form">
            <el-form-item label="类型分类：" :label-width="formLabelWidth">
              <el-radio-group v-model="dialogForm.typeid" size="small">
                <el-radio :label="1" border style="margin:0;">实物资源</el-radio>
                <el-radio :label="2" border>虚拟资源</el-radio>
                </el-radio-group>
            </el-form-item>
        </div>
      </el-form>
...
export default {
  ...
  methods:{
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Resource_typelist')){
              var operationsWidth = 22;
              if($this.menuButtonPermit.includes('Resource_typeedit')){
                operationsWidth+=66;
              }
              if($this.menuButtonPermit.includes('Resource_typedelete')){
                operationsWidth+=66;
              }
    ...
    handleClose(){
      ...
      $this.$store.dispatch('app/closeResourceType');
    },
    // 添加表格行数据
    addTableRow(){
      this.dialogFormVisible = true;
      this.dialogText = "添加资源类型";
      this.resetFormData();
    },
    // 编辑表格行数据
