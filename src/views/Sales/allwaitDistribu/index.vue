        ...
        <div slot="header">
            <div class="card-header">
                ...
                <div class="clues-info">
                    <p>                  
                        <span class="item-span-1">当前结果集状态：共有<strong>{{infoData.totalCount}}</strong>条。昨天共分配<strong>{{infoData.totalCount}}</strong>条询盘，当前共有<strong>{{infoData.totalCount}}</strong>条未处理，点击查看昨天未处理询盘。今天共分配<strong>{{infoData.totalCount}}</strong>条询盘，当前共有<strong>{{infoData.totalCount}}</strong>条未处理，点击查看今天未处理询盘。</span>                  
                    </p>
                </div>
                <div class="clues-info">
                    <p>
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        ...
        if(res.status){
          if(res.data.length>0){
            ...
            if($this.menuButtonPermit.includes('Sales_allwaitDistribu')){
              $this.initPage();
            }else{
              $this.$message({
                showClose: true,
