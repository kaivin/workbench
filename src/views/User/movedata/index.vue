          ...
          <div class="scroll-panel" ref="scrollDom" style="will-change:scroll-position">
              <div class="true-height" ref="scrollPane">
                <p class="breadcrumb" ref="breadcrumbPane">
                  ...
                  <span class="breadcrumb-link"><b>-</b><span>资源转移</span></span>
                </p>
                    ...
                    <div class="card-header" ref="headerPane">
                      <div class="move_dialog">
                        <div class="transfer-item">
                          ...
                          <el-select class="transfer-user" v-model="accountValue" clearable filterable placeholder="请选择">
                            <el-option
                        ...
                        <div class="transfer-btns">
                          <el-button class="resetBtn" size="small" @click="resetTrans">重置</el-button>
                          <el-button type="primary" size="small" :class="isSaveAccountData?'isDisabled':''" :disabled="isSaveAccountData" @click="saveAccountData">确 定</el-button>
                        </div>
                      </div>
                    </div>
                  </div>
                          ...
                          <el-table
                            ...
                            <el-table-column
                                ...
                                <template slot-scope="scope">
                                    ...
                                    <p><span class="EnColor05">富通ID：</span>{{scope.row.ftword_id}}</p>
                                    <p><span class="EnColor05">业务员：</span><strong>{{scope.row.name}}</strong></p>
                                    </div>
                                </template>
                            </el-table-column>
                            <el-table-column
                                ...
                                <template slot-scope="scope">
                                    ...
                                    <p>
                                      ...
                                      <span> 
                                          {{scope.row.custormemail}}
                                      </span>
                                    </p>
                                    <p><span class="EnColor05">电话：</span><span> {{scope.row.custormphone}}</span></p>
                                    </div>
                                </template>
                            </el-table-column>
                          </el-table>
...
export default {
  ...
  created(){
    ...
    $this.initData();
    $this.resourceUserId = $this.$route.query.id;
  },
  updated(){
    this.$nextTick(() => {
  ...
  methods:{
    ...
    initPage(){
      var $this = this;
      var formData = {}
      formData.id = $this.resourceUserId;
      formData.page = $this.searchData.page;
      formData.ftword_id = $this.searchData.ftword_id;
      formData.keyword = $this.searchData.keyword;
      formData.limit = $this.searchData.limit;
      document.getElementsByClassName("scroll-panel")[0].scrollTop = 0;
      $this.$store.dispatch('user/moveInquryAction', formData).then(response=>{
        if(response){
          if(response.status){
            ...
            $this.totalDataNum = response.count;
            setTimeout(()=>{
    ...
    saveAccountData(){
        ...
        if(!$this.isSaveAccountData){
          ...
          if($this.selectedData.length == 0 &&　$this.sourceList.length == 0){
            $this.$message({
              message: "请选择要转移的询盘",
              type: 'error',
            });
          ...
          resData.accpet_id = $this.accountValue;
