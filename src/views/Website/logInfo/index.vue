          ...
          <div class="ArticleSixFrBom" v-if="commentList.length>0">
            <div class="item-comment" v-for="item in commentList" v-bind:key="item.id">
              <div class="comment-header">
                ...
                <span v-if="articleData.commentdelete==1&&menuButtonPermit.includes('Article_commentdelete')" class="delete" v-on:click="deleteCommentPlug(item.id)" title="删除该条评论"><i class="el-icon-delete-solid"></i></span>
              </div>
...
<script>
import { websiteLogInfo,websiteLogReadUser } from '@/api/website';
...
export default {
  ...
  methods:{
    ...
    initPage(){
      ...
      websiteLogInfo({id:$this.currentID}).then(response=>{
          if(response){
            if(response.status){
    ...
    getReadUser(){
      var $this = this;
      websiteLogReadUser({id:$this.currentID}).then(response=>{
          if(response){
            if(response.status){
    ...
    logEdit(){
      var $this = this;
      var routeUrl =  $this.$router.resolve({path:'/Website/logAddEdit',query:{logID:$this.currentID,websiteID:$this.websiteID,website:$this.website}});
      window.open(routeUrl.href,'_self');
    },
    // 删除评论
    deleteCommentPlug(id){
      var $this = this;
