        ...
        <div class="item-filter flex-box group">
          ...
          <div class="filter-content flex-content">
            <div class="item-list group">
              <div class="item-checkbox" v-bind:class="item.isOn?'active':''" v-if="item.dept_id==12" v-for="item in groupList" v-bind:key="item.userid" v-on:click="groupChangeHandler(item.userid)"><i></i><span>{{item.typename}} {{item.groupName}}组</span><b>[{{item.departName}}]</b></div>
            </div>
            <div class="item-list group">
              <div class="item-checkbox" v-bind:class="item.isOn?'active':''" v-if="item.dept_id==13||item.dept_id==16" v-for="item in groupList" v-bind:key="item.userid" v-on:click="groupChangeHandler(item.userid)"><i></i><span>{{item.typename}} {{item.groupName}}组</span><b>[{{item.departName}}]</b></div>
            </div>
            <div class="item-list group">
      ...
      <div class="contrast-content flex-content">
        <div class="item-list">
          ...
          <template v-if="selectedData.contrastGroupID.length>0">
            <div class="item-contrast" v-if="item.isOn" v-for="item in contrastGroupList" v-bind:key="item.userid"><span>{{item.typename}} {{item.groupName}}组</span><i v-on:click="deleteContrastGroup(item.userid)" class="svg-i"><svg-icon icon-class="close" class-name="disabled" /></i></div>
          </template>
  ...
  methods: {
    ...
    getUserMenuButtonPermit(){
      ...
      $this.$store.dispatch('api/getMenuButtonPermitAction',{id:$this.$router.currentRoute.meta.id}).then(res=>{
        if(res.data.length>0){
          ...
          if(permitData.includes('Api_engroupcountdefault')){
            $this.initData()
          }else{
            $this.$message({
              showClose: true,
      ...
      $this.$store.dispatch('teamMember/enGroupDefaultAction', null).then(response=>{
          if(response){
            if(response.status){
                ...
                semData.historyMaxNumber = response.semhistorymaxnumber && response.semhistorymaxnumber.length > 0 ? numSeparate(response.semhistorymaxnumber[0].effectivenumber) : 0;
                semData.historyMaxNumberDate = response.semhistorymaxnumber && response.semhistorymaxnumber.length > 0 ? response.semhistorymaxnumber[0].xundate : "";
                semData.mainData = [];
                semData.totalChart = [];
                semData.colorArr = [];
                ...
                semTong.forEach(function(item,index){
                  var itemChart = {};
                  var newItemArr=[];
                  if(item[0].name.indexOf("组")!=-1){
                    itemChart.name = item[0].name
                  }else{
                ...
                seoData.historyMaxNumber = response.seohistorymaxnumber && response.seohistorymaxnumber.length > 0 ? numSeparate(response.seohistorymaxnumber[0].effectivenumber) : 0;
                seoData.historyMaxNumberDate = response.seohistorymaxnumber && response.seohistorymaxnumber.length > 0 ? response.seohistorymaxnumber[0].xundate : "";
                seoData.mainData = [];
                seoData.totalChart = [];
                seoData.colorArr = [];
                ...
                snsData.historyMaxNumber = response.snshistorymaxnumber && response.snshistorymaxnumber.length > 0 ? numSeparate(response.snshistorymaxnumber[0].effectivenumber) : 0;
                snsData.historyMaxNumberDate = response.snshistorymaxnumber && response.snshistorymaxnumber.length > 0 ? response.snshistorymaxnumber[0].xundate : "";
                snsData.mainData = [];
                snsData.totalChart = [];
                snsData.colorArr = [];
      ...
      if(!obj.isOn){
        ...
        contrastList.forEach(function(item,index){
          if($this.selectedData.isMonth){
            ...
            if(item.value=="dealScore"||item.value=="dealScoreRegion"||item.value=="dealCount"||item.value=="dealCountRegion"||item.value=="product"){
              item.isOn = false;
              item.disabled = true;
            }else{
              item.disabled = false;
    ...
    resetData(){
      ...
      if($this.$route.query.type){
        $this.$router.push({path:'/stat/en/groupAnalysis'});
      }else{
        $this.channelType = "";
        $this.groupName = "";
          ...
          if($this.selectedData.contrastGroupID.length>0){
            ...
            if($this.selectedData.groupID.length==1){
              $this.groupList.forEach(function(item){
                if(item.userid == $this.selectedData.groupID[0]){
                  if($this.selectedData.isMonth){
                    dealScoreData.chartTitle = "总成交积分月趋势";
                  }else{
                    dealScoreData.chartTitle = "总成交积分日趋势";
                  }
                  dealScoreData.name = "总成交积分";
                }
              });
            }else{// 多小组被选中
              if($this.selectedData.isMonth){
        ...
        if($this.selectedData.isDateCompare&&$this.selectedData.dateContrast&&$this.selectedData.dateContrast.length>0){
          ...
          if($this.selectedData.groupID.length==1){
            $this.groupList.forEach(function(item){
              if(item.userid == $this.selectedData.groupID[0]){
                if($this.selectedData.isMonth){
                  dealCountData.chartTitle = item.departName+"-"+item.groupName+"组成交个数日期对比月趋势";
                }else{
                  dealCountData.chartTitle = item.departName+"-"+item.groupName+"组成交个数日期对比日趋势";
                }
                dealCountData.name = item.departName+"-"+item.groupName+"组成交个数统计";
              }
            });
          }else{
            if($this.selectedData.isMonth){
              dealCountData.chartTitle = "小组组合成交个数日期对比月趋势";
            }else{
              dealCountData.chartTitle = "小组组合成交个数日期对比日趋势";
            }
            dealCountData.name = "小组组合成交个数统计";
          }
        }else{
          // 小组对比
          if($this.selectedData.contrastGroupID.length>0){
            if($this.selectedData.isMonth){
              dealCountData.chartTitle = "各小组成交个数月趋势对比";
            }else{
              dealCountData.chartTitle = "各小组成交个数日趋势对比";
            }
            ...
            if($this.selectedData.groupID.length==1){
              ...
              if($this.selectedData.isMonth){
                dealCountData.chartTitle = "小组组合成交个数月趋势";
              }else{
                dealCountData.chartTitle = "小组组合成交个数日趋势";
              }
              dealCountData.name = "小组组合成交个数统计";
              dealCountData.totalChart = [];
    ...
    mapChartDataClump(res){
      ...
      if($this.selectedType.includes("inquiryCountRegion")){
        ...
        if($this.selectedData.isDateCompare&&$this.selectedData.dateContrast&&$this.selectedData.dateContrast.length>0){
          ...
          if($this.selectedData.contrastGroupID.length>0){
            ...
            inquiryMapData.title = "询盘 - 国家排行榜";
            // inquiryMapData.title = "询盘地图展示 - 国家排行榜";
            inquiryMapData.unit = "（单位：个）";
            inquiryMapData.isMap = true;
          }else{
            inquiryMapData = [];
            ...
            // itemMapData.title = "询盘地图展示 - 国家排行榜";
      ...
      if($this.selectedType.includes("dealScoreRegion")){
        ...
        if($this.selectedData.isDateCompare&&$this.selectedData.dateContrast&&$this.selectedData.dateContrast.length>0){
          ...
          itemMapChart1.topTenColor = TopTenColor(itemMapChart1.topTenData,itemMapChart1.colorData);
          // dealRegionMapData.title = "成交积分地图展示 - 国家排行榜";
      ...
      if($this.selectedType.includes("dealCountRegion")){
        ...
        if($this.selectedData.isDateCompare&&$this.selectedData.dateContrast&&$this.selectedData.dateContrast.length>0){
          ...
          if($this.selectedData.contrastGroupID.length>0){
            ...
            // dealCountMapData.title = "成交个数地图展示 - 国家排行榜";
            dealCountMapData.title = "成交个数 - 国家排行榜";
            dealCountMapData.unit = "（单位：个）";
            dealCountMapData.isMap = true;
          }else{
