  ...
  <div class="page-root scroll-panel product-Analy" ref="boxPane">
        ...
        <div class='product-AnalyMain'> 
            ...
            <div class="enquirieTag" :class="[contrastName=='timeCont'?'TwoTagTop':'']">
                ...
                <div class="proAccount" :class="[EnquirieMap.length>1?'mapOneAccount':'',EnquirieMap.length>2?'mapTwoAccount':'']">
                        ...
                        :is-cn="false"
                        ></map-chart>
                        <div class="promapBoxFr" v-if="ChartMap.length>0">
                            <div class="enpromapBoxItem" :style="'width:'+1/EnquirieMap.length*100+'%'" v-for="(item,index) in ChartMap">
                                <tapmap-chart
                                :worldMapArr="item"
                                ></tapmap-chart>
                            </div>
                        ...
                        <div class="pro_user" id="pro_user" v-if="intableData.length>0||scoretableData.length>0">
                            <inquiry-table :intableData="intableData" :chooseData="chooseData" :lang="en"></inquiry-table>
                            <score-table :scoretableData="scoretableData" :chooseData="chooseData" :lang="en"></score-table>
                        </div>
                </div>
            </div>
        </div>
                    ...
                    if(res.provincecountmap&&res.provincecountmap.length>0){
                        ...
                        res.provincecountmap.forEach(function(item,index){
                            if(item&&item.length>0){
                                ...
                                EnquirieMapArr=EnquirieMapArr.concat($this.mapPlug(item,'询盘个数',productname+'询盘地区',maxNum));
                            }
                        });
                                            ...
                                            if(items&&items.length>0){
                                                if(item[0].departname==items[0].departname){
                                                    ...
                                                    if($this.mapNum>2){
                                                        ...
                                                        objItem01.mapWidth = mapWidth;
                                                        objItem01.mapHeight = mapHeight;
                                                        objItem02.mapWidth = mapWidth;
                                                        objItem02.mapHeight = mapHeight;
                                                    }
                                                    objItem01.randomStr = randomString(4);
